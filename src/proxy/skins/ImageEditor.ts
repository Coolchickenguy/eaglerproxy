...
export namespace ImageEditor {
  ...
  export async function loadLibraries(native: boolean) {
    ...
    if (native) sharp = (await import("sharp")).default;
      ...
      try {
        Jimp = {
          ...(await import("jimp" as any)),
          // Replace the default constructor with a custom one
          Jimp: class CustomBitmapConstructor extends (await import("jimp" as any)).Jimp {
            constructor(bitmap) {
              if (
                bitmap &&
                bitmap.width != null &&
        ...
        logger.fatal(
          "Please ensure that Jimp is installed by running 'npm install jimp' in the terminal.",
        );
        logger.fatal(
          "If you'd like to use the faster native image editor, please set the 'useNatives' option in the config to true.",
        );
  ...
  export async function copyRawPixelsJS(
    imageIn: Jimp,
    imageOut: Jimp,
    dx1: number,
    dy1: number,
  ...
  export async function toEaglerSkinJS(
    image: Buffer,
  ): Promise<
    Util.BoundedBuffer<typeof Constants.EAGLERCRAFT_SKIN_CUSTOM_LENGTH>
  > {
    let jimpImage = await Jimp.Jimp.read(image),
      height = jimpImage.height;
    if (height != 64) {
      // assume 32 height skin
      let imageOut = new Jimp.Jimp({width: 64, height: 4, color: 0x0});

      for (let x = 0; x < jimpImage.width; x++) {
        for (let y = 0; y < jimpImage.height; y++) {
          imageOut.setPixelColor(jimpImage.getPixelColor(x, y), x, y);
        }
      }

      imageOut = await copyRawPixelsJS(
        jimpImage,
        imageOut,
        24,
        48,
  ...
  export function generateEaglerMOTDImage(
    file: string | Buffer,
  ): Promise<Buffer> {
  ...
  export async function generateEaglerMOTDImageJS(
    file: string | Buffer,
  ): Promise<Buffer> {
    const jimpImage = await Jimp.Jimp.read(
      typeof file == "string" ? await fs.readFile(file) : file);
    const resizedImage = jimpImage.resize({
      w: Constants.ICON_SQRT,
      h: Constants.ICON_SQRT,
      strategy: Jimp.ResizeStrategy.RESIZE_NEAREST_NEIGHBOR
    });
    return resizedImage.bitmap.data;
  }
}
